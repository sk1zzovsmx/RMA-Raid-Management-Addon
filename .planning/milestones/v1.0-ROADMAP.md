...
**Key decisions:** Preserve incompatible raid archives through quarantine; use canonical map-ID recognition as the sole raid-admission gate; add owner-local request admission limits without changing R5 wire formats; distinguish automated, observed, and deferred verification evidence.

**Issues resolved:** Destructive future-format archive handling, locale-dependent raid admission, incomplete localized yell fallback, unbounded request-driven reserve/distribution response work, stale validation fixtures, and the remaining proprietary non-ASCII comment.

**Issues deferred:** Real-client SavedVariables quarantine/recovery, localized multi-client synchronization, combat protected-action behavior, taint verification, and proof of branding predating the available normalized Git history.

**Technical debt:** Historical Phase 1 and Phase 2 verification wording overstates live approval; the Phase 4 disposition and milestone audit are authoritative.
