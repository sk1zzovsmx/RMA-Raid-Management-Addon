...
class InitCoreMicroCleanupContractTest(unittest.TestCase):
    ...
    def test_init_depends_on_wotlk_group_rank_apis_without_addon_overrides(self):
        src = read(INIT)
        self.assertIn("local UnitIsGroupLeader = assert(", src)
        self.assertIn("_G.UnitIsGroupLeader", src)
        self.assertIn('"RMA group leader API is not initialized"', src)
        self.assertIn("local UnitIsGroupAssistant = assert(", src)
        self.assertIn("_G.UnitIsGroupAssistant", src)
        self.assertIn('"RMA group assistant API is not initialized"', src)
        self.assertNotIn("local UnitIsGroupLeader = _G.UnitIsGroupLeader", src)
        self.assertNotIn("local UnitIsGroupAssistant = _G.UnitIsGroupAssistant", src)
        self.assertNotIn("addon.UnitIsGroupLeader", src)
        self.assertNotIn("addon.UnitIsGroupAssistant", src)
        self.assertNotIn("if groupLeader and groupLeader(unit) then", src)
        self.assertNotIn("if groupAssistant and groupAssistant(unit) then", src)
    ...
    def test_comms_depends_on_wotlk_send_and_player_name_apis_without_local_fallbacks(self):
        ...
        self.assertIn("local RegisterAddonMessagePrefix =", comms)
        self.assertIn("assert(_G.RegisterAddonMessagePrefix", comms)
        self.assertIn("_G.RegisterAddonMessagePrefix", comms)
        self.assertIn('"Comms addon-message prefix registration API is not initialized"', comms)
        ...
        self.assertNotIn('if type(register) == "function" then', comms)
    ...
    def test_reserves_sync_depends_on_comms_without_send_fallback(self):
        ...
        self.assertIn("local RegisterAddonMessagePrefix =", reserves_sync)
        self.assertIn("assert(_G.RegisterAddonMessagePrefix", reserves_sync)
        self.assertIn("_G.RegisterAddonMessagePrefix", reserves_sync)
        self.assertIn('"Reserves sync prefix registration API is not initialized"', reserves_sync)
    ...
    def test_loot_distribution_session_depends_on_wotlk_time_api_without_local_fallbacks(self):
        ...
        self.assertIn("local RegisterAddonMessagePrefix =", distribution)
        self.assertIn("assert(_G.RegisterAddonMessagePrefix", distribution)
        self.assertIn('"Loot distribution prefix registration API is not initialized"', distribution)
        ...
        self.assertNotIn('if type(register) == "function" then', distribution)
    ...
    def test_loot_context_uses_declared_database_owner_without_optional_core_guards(self):
        ...
        self.assertIn("local GetCurrentRaid = assert(", loot_context)
