...
class InitCoreMicroCleanupContractTest(unittest.TestCase):
    def test_attendance_export_owns_raid_attendance_csv_without_frame_access(self):
        ...
        self.assertIn('addon.Database.EnsureServiceNamespace("Attendance", "Export")', attendance_export)
        self.assertIn("function Export:BuildCSV(raid, context)", attendance_export)
    ...
    def test_logger_export_no_longer_owns_raid_attendance_csv(self):
        ...
        self.assertNotIn("function Export:GetCSV(mode, raid, context)", logger_export)
        self.assertIn("function Export:BuildCSV(raid, context)", logger_export)
        self.assertNotIn("function Export:GetLootCSV", logger_export)
    ...
    def test_csv_exporters_share_one_row_encoder(self):
        strings = read(STRINGS)
        logger_export = read(LOGGER_EXPORT)
        attendance_export = read(ATTENDANCE_EXPORT)

        self.assertIn("function Strings.AppendCSVRow(lines, fields, encoded, fieldCount)", strings)
        for source in (logger_export, attendance_export):
            self.assertIn("local AppendCSVRow = assert(Strings.AppendCSVRow", source)
            self.assertNotIn("local function encodeCSVField", source)
            self.assertNotIn("local function csvEscape", source)
            self.assertNotIn("local function appendCSVLine", source)

    def test_shared_csv_row_encoder_preserves_csv_values(self):
        script = textwrap.dedent(
            f"""
            _G.GetAchievementLink = function() return nil end
            local addon = {{ Colors = {{}} }}
            assert(loadfile("{str(STRINGS).replace(chr(92), chr(92) * 2)}"))("RMA", addon)
            local lines = {{}}
            addon.Strings.AppendCSVRow(lines, {{ nil, false, 'a,b', 'a"b', "a\\nb" }}, {{}}, 5)
            assert(lines[1] == ',false,"a,b","a""b","a\\nb"')
            """
        )
        subprocess.run(
    ...
    def test_database_spine_does_not_export_savedvariables_manager_facade(self):
        ...
        self.assertIn("function Database.GetRaidStore()", db)
        self.assertIn('assert(type(raidStore) == "table", "RMA RaidStore is not initialized")', db)
        self.assertNotIn("GetRaidStoreOrNil", db)
    ...
    def test_raid_migrations_depend_on_loot_source_candidates_without_local_fallback(self):
        ...
        self.assertIn("addon.LootSourceCandidates", migrations)
    ...
    def test_raid_migrations_depend_on_strings_without_local_normalization_fallbacks(self):
        ...
        self.assertIn("addon.Strings", migrations)
    ...
    def test_db_options_emits_events_through_bus_without_optional_sender_guard(self):
        ...
        self.assertIn("local Bus = addon.Bus", options)
    ...
    def test_db_sync_payload_uses_validated_raid_store_methods_without_optional_method_guards(self):
        ...
        self.assertIn("local raidStore = Database.GetRaidStore()", sync_payload)
    ...
    def test_db_sync_import_uses_validated_raid_store_revision_methods_without_optional_method_guards(self):
        ...
        self.assertIn("Database.GetRaidStore():SetRaidSyncRevision(raid,", sync_import)
        self.assertIn("local raidStore = Database.GetRaidStore()", sync_import)
    ...
    def test_db_syncer_binds_persistent_sync_callbacks_without_optional_event_fallbacks(self):
        ...
        self.assertIn("local BuildConfigOptionChangedName =", syncer)
        self.assertIn("Events.BuildConfigOptionChangedName", syncer)
        ...
        self.assertIn('local persistentSyncEvent = BuildConfigOptionChangedName("persistentSync")', syncer)
        ...
        self.assertNotIn("Events.BuildConfigOptionChangedName and Events.BuildConfigOptionChangedName", syncer)
    ...
    def test_init_forwards_roster_delta_through_validated_internal_event(self):
        ...
        self.assertIn("local Events = addon.Events", init)
        self.assertIn("local Bus = addon.Bus", init)
    ...
    def test_spec_inspect_binds_and_emits_events_without_optional_bus_guards(self):
        ...
        self.assertIn('Events.ResolveWowForwardedName("READY_CHECK")', spec_inspect)
    ...
    def test_equip_inspect_binds_declared_events_without_optional_callback_guards(self):
        ...
        self.assertRegex(equip_inspect, r"local\s+ResolveWowForwardedName\s*=\s*assert\(")
        self.assertIn("Events.ResolveWowForwardedName", equip_inspect)
        ...
        self.assertIn('assert(ResolveWowForwardedName("INSPECT_TALENT_READY")', equip_inspect)
        self.assertIn('"EquipInspect talent-ready event is not initialized"', equip_inspect)
        self.assertIn('assert(ResolveWowForwardedName("PLAYER_REGEN_ENABLED")', equip_inspect)
        ...
        self.assertNotIn("Events.ResolveWowForwardedName(\"INSPECT_TALENT_READY\")", equip_inspect)
    ...
    def test_master_controller_registers_bus_callbacks_through_validated_listener(self):
        ...
        self.assertRegex(master, r"local\s+ResolveWowForwardedName\s*=\s*assert\(")
        self.assertIn("Events.ResolveWowForwardedName", master)
        ...
        self.assertIn("RegisterCallback(ResolveWowForwardedName(methodName)", master)
        self.assertNotIn("Bus.RegisterCallback(InternalEvents.", master)
        self.assertNotIn("Bus.RegisterCallback(Events.ResolveWowForwardedName and Events.ResolveWowForwardedName", master)
    ...
    def test_master_trade_uses_declared_raid_store_without_optional_guard(self):
        ...
        self.assertIn("return Database.EnsureRaidByIndex(raidNum)", trade)
        self.assertNotIn("if Database.EnsureRaidByIndex then", trade)
    ...
    def test_logger_actions_uses_raid_store_database_facade_without_store_lookup_fallbacks(self):
        ...
        self.assertIn("return Database.GetRaidNidByIndex(currentRaid)", actions)
        self.assertIn("restoreCurrentRaidIndex = function(currentRaidNid)", actions)
        self.assertIn("local currentRaidId = Database.GetRaidIndexByNid(currentRaidNid)", actions)
    ...
    def test_logger_actions_uses_declared_raid_store_owner_without_optional_getter_guards(self):
        ...
        self.assertIn("Database.GetRaidStore():MarkLootSyncRevision(raid, loot, reason or \"loot\")", actions)
        self.assertIn("Database.GetRaidStore():TouchRaidSyncRevision(raid, reason or \"loot\")", actions)
        self.assertIn("local raidStore = Database.GetRaidStore()", actions)
        self.assertNotIn("GetRaidStoreOrNil", actions)
    ...
    def test_config_controller_publishes_option_changes_without_optional_event_guards(self):
        ...
        self.assertRegex(config, r"local\s+BuildConfigOptionChangedName\s*=\s*assert\(")
        self.assertIn("Events.BuildConfigOptionChangedName", config)
        self.assertIn('"Config option-change event resolver is not initialized"', config)
        self.assertIn("local eventName = BuildConfigOptionChangedName(key)", config)
        self.assertIn("local eventName = BuildConfigOptionChangedName(name)", config)
        self.assertIn("TriggerEvent(eventName, value)", config)
        self.assertNotIn("Events.BuildConfigOptionChangedName and Events.BuildConfigOptionChangedName", config)
    ...
    def test_loot_and_raid_services_use_declared_raid_store_cache_owner_without_manual_fallbacks(self):
        ...
        self.assertIn("Database.GetRaidStore():UpsertLootIndex(raid, lootInfo, index)", loot_service)
        self.assertIn("Database.StripRuntimeRaidCaches(raid)", raid_state)
        for source in (loot_service, raid_state):
    ...
    def test_slash_events_uses_database_owners_without_string_dispatch_helper(self):
        ...
        self.assertIn("local raids = Database.GetRaidStore():GetAllRaids()", slash)
        self.assertIn("if currentRaid then", slash)
        self.assertIn("raidNid = Database.GetRaidStore():GetRaidNidByIndex(currentRaid)", slash)
        ...
        self.assertNotIn("GetRaidStoreOrNil", slash)
    ...
    def test_loot_context_uses_declared_database_owner_without_optional_core_guards(self):
        ...
        self.assertIn("local EnsureRaidByIndex = assert(", loot_context)
        self.assertIn("Database.EnsureRaidByIndex", loot_context)
        ...
        self.assertIn("local raid = EnsureRaidByIndex(resolvedRaidNum)", loot_context)
        self.assertIn("EnsureRaidSchema(raid)", loot_context)
        self.assertNotIn("core and type(core.GetCurrentRaid)", loot_context)
        self.assertNotIn('type(core.EnsureRaidByIndex) ~= "function"', loot_context)
    ...
    def test_loot_rules_depends_on_ignored_items_dataset_without_optional_fallbacks(self):
        ...
        self.assertIn("addon.IgnoredItems", loot_rules)
        ...
        self.assertNotIn('addon.IgnoredItems or {}', loot_rules)
    ...
    def test_raid_loot_method_uses_declared_database_and_creature_id_owners_without_optional_fallbacks(self):
        ...
        self.assertIn("addon.GetCreatureId", loot_method)
    ...
    def test_raid_session_depends_on_dboptions_without_local_fallback(self):
        ...
        self.assertIn("local isDebugEnabled = addon.Options.IsDebugEnabled", raid_session)
    ...
    def test_raid_roster_depends_on_dboptions_without_local_fallback(self):
        ...
        self.assertIn("local isDebugEnabled = addon.Options.IsDebugEnabled", raid_roster)
    ...
    def test_raid_counts_emits_player_count_changed_through_validated_publisher(self):
        ...
        self.assertNotIn("local bus = addon.Bus", raid_counts)
    ...
    def test_raid_state_depends_on_dboptions_without_local_fallback(self):
        ...
        self.assertIn("local isDebugEnabled = addon.Options.IsDebugEnabled", raid_state)
    ...
    def test_raid_state_uses_declared_raid_store_lookup_without_optional_function_guard(self):
        ...
        self.assertIn("local raid = Database.EnsureRaidByIndex(raidNum)", raid_state)
        self.assertIn("local queryRaidNum = tonumber(raidNum) or tonumber(Database.GetCurrentRaid()) or 0", raid_state)
        self.assertIn("local raidStore = Database.GetRaidStore()", raid_state)
        self.assertNotIn(
            'type(Database.EnsureRaidByIndex) == "function" and Database.EnsureRaidByIndex(raidNum) or nil',
            raid_state,
        )
        self.assertNotIn("Database.GetCurrentRaid and Database.GetCurrentRaid()", raid_state)
        self.assertNotIn("GetRaidStoreOrNil", raid_state)
    ...
    def test_loot_service_uses_declared_raid_store_owner_without_optional_getter_guard(self):
        ...
        self.assertIn("return Database.GetRaidStore():UpsertLootIndex(raid, lootInfo, index)", loot_service)
        self.assertNotIn("GetRaidStoreOrNil", loot_service)
    ...
    def test_loot_recording_uses_declared_raid_store_owner_without_optional_getter_guard(self):
        ...
        self.assertIn('Database.GetRaidStore():MarkLootSyncRevision(raid, row, "loot_row")', recording)
        ...
        self.assertNotIn("GetRaidStoreOrNil", recording)
    ...
    def test_raid_state_uses_declared_creature_id_owner_without_optional_fallback(self):
        ...
        self.assertIn("addon.GetCreatureId", raid_state)
    ...
    def test_chat_service_uses_group_policy_owner_without_local_roster_fallbacks(self):
        ...
        self.assertIn("addon.GetGroupTypeAndCount", chat)
    ...
    def test_reserves_service_uses_declared_database_raid_owners_without_optional_guards(self):
        ...
        self.assertIn("return Database.EnsureRaidByIndex(currentRaid)", reserves_service)
        ...
        self.assertNotIn("if Database and Database.EnsureRaidByIndex then", reserves_service)
