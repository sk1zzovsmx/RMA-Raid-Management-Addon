...
local function sendAddonMessageNow(prefix, msg, channel, target)
    local sender = _G.SendAddonMessage
    ...
    if type(target) == "string" and target ~= "" then
        ok = pcall(sender, prefix, tostring(msg), channel, target)
    else
        ok = pcall(sender, prefix, tostring(msg), channel)
    end
    return ok == true
end

local function scheduleAddonQueueFlush()
    if type(Comms.ScheduleTimer) ~= "function" or type(Comms.CancelTimer) ~= "function" then
        return false
    end
    ...
    Comms._addonQueueTimer = Comms:ScheduleTimer(function()
...
function Comms.QueueAddonMessage(prefix, msg, channel, target, opts)
    ...
    if not (opts and opts.immediate == true) and type(Comms.ScheduleTimer) == "function" and type(Comms.CancelTimer) == "function" then
