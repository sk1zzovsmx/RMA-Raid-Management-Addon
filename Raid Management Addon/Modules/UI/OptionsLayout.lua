...
local function applyCommandRow(frame, row, cfg, cursorY, titleHeight, bodyGap)
	...
	placeButton(
		frame,
		resolveWidget(frame, row.button),
		commandX,
		cursorY + (row.buttonYOffset or -4),
		commandWidth,
		buttonHeight
	)
	return height
...
local function applyEditCommandRow(frame, row, cfg, cursorY, titleHeight, bodyGap)
	...
	place(
		frame,
		resolveWidget(frame, row.editBox),
		commandX,
		cursorY + (row.editYOffset or -2),
		commandWidth,
		editHeight
	)
	placeButton(
		...
		resolveWidget(frame, row.button),
		commandX,
		cursorY - editHeight - (row.buttonGap or 8),
		commandWidth,
		buttonHeight
	)
	return height
...
local function applyDropDownRow(frame, row, cfg, cursorY, titleHeight, bodyGap)
	...
	if dropDown then
		UIDropDownMenu_SetWidth(dropDown, row.dropdownTextWidth or commandWidth - 33)
		UIDropDownMenu_SetButtonWidth(dropDown, row.dropdownButtonWidth or commandWidth - 13)
	end
	return height
end
...
local function applyButtonRow(frame, row, cfg, cursorY)
	...
	for i = 1, #buttons do
		placeButton(
			frame,
			resolveWidget(frame, buttons[i]),
			leftX + ((i - 1) * (buttonWidth + gap)),
			cursorY,
			buttonWidth,
			buttonHeight
		)
...
local function applyRow(frame, row, cfg, cursorY)
	...
	elseif rowType == "title" then
		...
		placeText(
			frame,
			resolveWidget(frame, row.suffix),
