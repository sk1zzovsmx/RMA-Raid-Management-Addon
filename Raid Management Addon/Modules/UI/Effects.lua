...
local function stopProcGlow(glow)
	SetScriptSafely(glow.procFrame, "OnUpdate", nil)
...
local function startProcGlow(glow, settings, r, g, b)
	...
	procFrame:Show()
	SetScriptSafely(procFrame, "OnUpdate", onProcGlowUpdate)
...
local function onTimedFadeUpdate(frame, elapsed)
	...
	if not fade then
		if frame then
			SetScriptSafely(frame, "OnUpdate", nil)
		end
		return
	end
...
local function ensureGlow(button)
	...
	Frames.HookScriptSafely(button, "OnSizeChanged", function(self)
