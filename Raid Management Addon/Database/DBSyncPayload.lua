...
function SnapshotPayload.DecodeTransportText(value)
    ...
    if input:sub(1, #COMPRESSED_PREFIX) == COMPRESSED_PREFIX then
        local lib = getLibDeflate()
        if not (lib and type(lib.DecodeForWoWAddonChannel) == "function" and type(lib.DecompressDeflate) == "function") then
            return nil
        end
...
function SnapshotPayload.Build(raid)
    ...
    local revision = raidStore and raidStore.GetRaidSyncRevision and raidStore:GetRaidSyncRevision(raid) or 0
...
function SnapshotPayload.BuildDelta(raid, sinceRevision)
    ...
    local raidStore = Database.GetRaidStoreOrNil("DBSyncPayload.BuildDelta", { "GetRaidSyncRevision", "GetLootSyncRevision", "RequiresFullSyncSince" })
    local fromRevision = tonumber(sinceRevision) or 0
    if fromRevision < 0 then
        fromRevision = 0
    end
    if raidStore and raidStore.RequiresFullSyncSince and raidStore:RequiresFullSyncSince(raid, fromRevision) then
        return nil, 0
    end
    ...
    for i = 1, #lootRows do
        ...
        local rowRevision = raidStore and raidStore.GetLootSyncRevision and raidStore:GetLootSyncRevision(raid, row) or tonumber(row and row.syncRevision) or 0
