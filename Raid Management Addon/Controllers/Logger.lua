...
local LOOT_LAYOUT_COLUMNS = {
    ...
    { headerSuffix = "HeaderSource", rowKey = "Source", hitBoxKey = "SourceHitBox", widthKey = "source", trailingGap = true, sortKey = "source" },
...
local function applyRaidListColumnWidths(frameName)
    ...
    applyLoggerHeaderColumnWidths(frameName, widths, RAID_LAYOUT_COLUMNS, LoggerLayout.LOGGER_PANEL_SCROLL_LEFT_OFFSET + LoggerLayout.LOGGER_ROW_LEFT_INSET)
end
...
local function applyAttendanceListColumnWidths(frameName)
    ...
    applyLoggerHeaderColumnWidths(frameName, widths, columns, LoggerLayout.LOGGER_PANEL_SCROLL_LEFT_OFFSET + LoggerLayout.LOGGER_ROW_LEFT_INSET)
end
...
do
    ...
    function uiState.AcquireRefs(frame)
        return {
            ...
            raids = UI.Frames.GetRef(frame, "RMALootHistoryRaids"),
            loot = UI.Frames.GetRef(frame, "RMALootHistoryLoot"),
        }
    end
    ...
    local function getExportFrameRefs()
        local frame = UI.Frames.Get("RMAExportFrame")
        if not frame then
            return nil
        end
    ...
    do
        ...
        local function getItemMenuFrame()
            return _G.RMALootHistoryItemMenuFrame or CreateFrame("Frame", "RMALootHistoryItemMenuFrame", UIParent, "UIDropDownMenuTemplate")
        end
        ...
        openItemWinnerPopup = function()
            ...
            Popups.ShowEditBox("RMALOGGER_ITEM_EDIT_WINNER", L.StrEditItemLooterHelp, function(self, text)
        ...
        openItemRollValuePopup = function()
            ...
            Popups.ShowEditBox("RMALOGGER_ITEM_EDIT_VALUE", L.StrEditItemRollValueHelp, function(self, text)
    ...
    do
        ...
        function confirmDeleteSelectedRaids(btn)
            ...
            if btn and UI.Selection.GetCount(ctx) > 0 and Popups then
                Popups.ShowConfirm("RMALOGGER_DELETE_RAID", L.StrConfirmDeleteRaid, deleteRaids)
            end
        end
    end
...
do
    ...
    controller = makeLoggerList({
        keyName = "LootList",
        poolTag = "logger-loot",
        _rowParts = { "Name", "Source", "SourceHitBox", "Winner", "Type", "Roll", "Time", "ItemIconTexture", "ItemNormalTexture" },
    ...
    showSourceTooltip = function(widget)
        ...
        local model = buildSourceTooltipModel(widget._RMARow)
        if model then
    ...
    do
        ...
        confirmDeleteSelectedLootItems = function()
            if UI.Selection.GetCount(module._msLootCtx) > 0 and Popups then
                Popups.ShowConfirm("RMALOGGER_DELETE_ITEM", L.StrConfirmDeleteItem, deleteItem)
            end
        end
    end
    ...
    attendancePlayersController = makeLoggerList({
        ...
        localize = function(n)
            ...
            if frame and not frame._RMAAttendanceBound then
                bindLoggerSortHeaders(n, ATTENDANCE_INSPECT_LAYOUT_COLUMNS, attendancePlayersController, "_RMAAttendanceBound")
            end
        end,
