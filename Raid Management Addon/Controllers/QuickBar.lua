...
-- exports: addon.Controllers.QuickBar QuickBar action bar API
...
local Raid = assert(Services.Raid, "QuickBar raid service is not initialized")
local LoggerController = assert(Controllers.Logger, "QuickBar logger controller is not initialized")
local ReservesWidget = assert(Widgets.ReservesUI, "QuickBar reserves widget is not initialized")
local WarningsController = assert(Controllers.Warnings, "QuickBar warnings controller is not initialized")
local PartyLootMethodChanged = assert(
	Events.Wow.PartyLootMethodChanged,
	"QuickBar loot-method event is not initialized"
)

local _G = _G
local mathMax, mathMin = math.max, math.min

Controllers.QuickBar = Controllers.QuickBar or {}
local module = Controllers.QuickBar
...
local function requestWithConfirmation(method, popupKey, text)
	if Raid:GetLootMethodName() == method then
		return false
	end
...
function module:RefreshLootMethod(method)
	...
	Frames.SetShown(refs.MLGlow, method == "master" and self:IsButtonShown("ML"))
	Frames.SetShown(refs.GLGlow, method == "group" and self:IsButtonShown("GL"))
end
