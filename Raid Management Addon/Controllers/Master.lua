...
do
    ...
    -- ----- Internal state ----- --
    local getFrame = feature.MakeModuleFrameGetter(module, "RMAMaster")
    ...
    local function copyVisibleRollRows(out)
        ...
        for i = 1, #visibleRows do
            ...
            if source then
                local copy = MasterService.BuildRollListRow(source, i)
                if copy then
                    out[#out + 1] = copy
                end
            end
    ...
    local function completeManualTradeCloseSettle()
        manualTradeCloseSettleHandle = nil
        if MasterService.SettleManualTradeClose then
            MasterService.SettleManualTradeClose()
        end
    ...
    local function scheduleManualTradeCloseSettle()
        ...
        if not (MasterService.HasManualTradeClosePending and MasterService.HasManualTradeClosePending()) then
            if MasterService.ResetManualTrade then
                MasterService.ResetManualTrade(true, true)
            end
            UI.Widgets.Call("TradeMenu", "HideDropdowns")
            return false
        end
    ...
    local function failManualTrade(message)
        if MasterService.CancelManualTradeClose then
            local failed = MasterService.CancelManualTradeClose(message)
            if failed then
                cancelManualTradeCloseSettle()
                UI.Widgets.Call("TradeMenu", "HideDropdowns")
                return true
    ...
    Private.EnsureRaidGridConfirmPopup = function()
        ...
        if not StaticPopupDialogs.RMA_MASTER_LOOT_GRID_CONFIRM then
            local dialog = {
            ...
            if Popups and Popups.Define then
                Popups.Define("RMA_MASTER_LOOT_GRID_CONFIRM", dialog)
            else
                StaticPopupDialogs.RMA_MASTER_LOOT_GRID_CONFIRM = dialog
            end
        end
        return StaticPopupDialogs.RMA_MASTER_LOOT_GRID_CONFIRM
    end
    ...
    Private.ShowManualGridAwardConfirm = function(itemLink, playerName)
        ...
        if Popups and Popups.Show then
            Popups.Show("RMA_MASTER_LOOT_GRID_CONFIRM", data.itemText, playerName, data)
        end
        return true
    end
    ...
    function module:TRADE_ACCEPT_UPDATE(playerAccepted, targetAccepted)
        ...
        if isTraceEnabled() then
            addon:trace(Diag.D.LogTradeAcceptUpdate:format(tostring(lootState.trader), tostring(tradeWinner), tostring(playerAccepted), tostring(targetAccepted)))
        end
        ...
        if not RMATradeHandled and MasterService.ApplyManualTradeAccept then
            local _, manualState = MasterService.ApplyManualTradeAccept(playerAccepted, targetAccepted, isAddonDrivenTrade)
            if manualState then
    ...
    function module:TRADE_SHOW()
        cancelManualTradeCloseSettle()
        if MasterService.ResetManualTrade then
            MasterService.ResetManualTrade(true, false)
        end
