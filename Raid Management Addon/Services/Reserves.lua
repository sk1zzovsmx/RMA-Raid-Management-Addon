...
do
    ...
    function module:GetCounts(sourceData)
        ...
        return countReserves(RMA_Reserves)
    end
    ...
    local function saveCanonicalReservesData(canonical)
        ...
        RMA_Reserves = buildSavedReservesData(canonical)
    end
    ...
    function module:Load()
        if isDebugEnabled() then
            addon:debug(Diag.D.LogReservesLoadData:format(tostring(RMA_Reserves ~= nil)))
        end
        clearDisplayRefreshQueue()
        local savedReserves = (type(RMA_Reserves) == "table") and RMA_Reserves or {}
