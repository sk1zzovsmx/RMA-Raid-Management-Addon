...
local function getItemLink()
    local moduleRef = Loot
    local currentIndex = tonumber(feature.lootState.currentItemIndex) or 1
    if moduleRef and moduleRef.GetItemLink then
        return moduleRef.GetItemLink(currentIndex)
    end
    return nil
end
    ...
    for bag = 0, 4 do
        ...
        for slot = 1, n do
            ...
            if link then
                ...
                if matches then
                    hasMatch = true
                    if not Inventory.ModuleItemIsSoulbound(bag, slot) then
...
function Inventory.ResolveTradeableInventoryItem(itemLink, cachedBag, cachedSlot, selectedItemCount)
    ...
    if cachedBag and cachedSlot then
        ...
        if cachedLink then
            ...
            if sameItem and not Inventory.ModuleItemIsSoulbound(cachedBag, cachedSlot) then
...
function Inventory.ResolveTradeAwardedCount()
    ...
    if addon.hasDebug then
        addon:debug(debugDiag.LogTradeAwardedCountResolved:format(awarded, source, tostring(before), tostring(after), selected))
    end
    return awarded
end
...
function Inventory.ModuleItemIsSoulbound(bag, slot)
    return module.ItemIsSoulbound(bag, slot)
end
