...
local function sendDirect(channel, target, ...)
    ensurePrefix()
    if not (Comms and type(Comms.QueueAddonMessage) == "function") then
        return false
    end
    return Comms.QueueAddonMessage(PREFIX, packFields(...), channel, target)
end
...
local function handleSnapshotChunkMessage(fields, sender)
    ...
    if not (chunkIndex and chunkIndex > 0 and totalChunks and totalChunks > 0 and totalChunks <= MAX_SNAPSHOT_CHUNKS) then
        return true
    end
...
function DistributionSession.PublishSnapshot(target, requestId)
    ...
    for chunkIndex = 1, chunkCount do
        ...
        local sent
        if target and target ~= "" then
            sent = sendDirect("WHISPER", target, MSG_SNAPSHOT_CHUNK, PROTOCOL_VERSION, requestId or "", sessionId, chunkIndex, chunkCount, chunk)
        else
            sent = publishMessage(MSG_SNAPSHOT_CHUNK, PROTOCOL_VERSION, requestId or "", sessionId, chunkIndex, chunkCount, chunk)
        end
        if sent ~= true then
            sentAll = false
        end
