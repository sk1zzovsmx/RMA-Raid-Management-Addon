...
do
    ...
    local function findLootSlotIndex(itemLink)
        return Inventory.FindLootSlotIndex(itemLink)
    end

    local function scanTradeableInventory(itemLink, itemId)
        return Inventory.FindTradeableInventoryMatch(itemLink, itemId)
    end

    local function resolveTradeableInventoryItem(itemLink, cachedBag, cachedSlot, selectedItemCount)
        return Inventory.ResolveTradeableInventoryItem(itemLink, cachedBag, cachedSlot, selectedItemCount)
    end

    local function resolveTradeAwardedCount()
        return Inventory.ResolveTradeAwardedCount()
    end

    local function resolveInventoryAwardedCount()
        return Inventory.ResolveInventoryAwardedCount()
    end

    local function resolveInventoryAwardedCountFromArgs(selectedItemCount, fromInventory)
        return Inventory.ResolveInventoryAwardedCountFromArgs(selectedItemCount, fromInventory)
    end

    local function buildTradeNotificationPlan(args)
        return AwardPlanner.BuildTradeNotificationPlan(args)
    end

    local function buildAwardTargetPlan(args)
        return AwardPlanner.BuildAwardTargetPlan(args)
    end

    local function validateInventoryTradeSelection(args)
        return AwardPlanner.ValidateInventoryTradeSelection(args)
    end

    local function buildMultiAwardWinnersPlan(args)
        return AwardPlanner.BuildMultiAwardWinnersPlan(args)
    end

    local function buildMultiAwardState(args)
        return AwardPlanner.BuildMultiAwardState(args)
    end

    local function buildMultiAwardSlotCandidates(itemLink)
        return Inventory.BuildMultiAwardSlotCandidates(itemLink)
    end
    ...
    local function appendLootRecord(raid, raidNum, lootInfo, options)
        ...
        if options.consumeLootWindow and lootState.opened == true and lootState.fromInventory ~= true and raidService and raidService.ConsumeLootWindowItemContext then
