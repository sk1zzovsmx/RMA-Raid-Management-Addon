...
do
    ...
    local function ensureRealmPlayerMeta(realm)
        RMA_Players[realm] = RMA_Players[realm] or {}
        return RMA_Players[realm]
    end
    ...
    function module:GetPlayerClass(name)
        ...
        if RMA_Players[realm] and RMA_Players[realm][resolvedName] then
            class = RMA_Players[realm][resolvedName].class or "UNKNOWN"
        end
        return class
    end
