...
do
    ...
    local LootContext = assert(LootService.GetContextBridge and LootService:GetContextBridge(), "Loot context bridge is not initialized")
    ...
    local function createLootWindowItemSnapshot(raidNum, bossNid, items, source, now, ttlSeconds)
        return LootContext.CreateSnapshot(raidState, raidNum, bossNid, items, source, now, ttlSeconds, LOOT_WINDOW_BOSS_CONTEXT_TTL_SECONDS)
    end

    local function setActiveLootWindowItemSnapshot(raid, raidNum, snapshot, now, ttlSeconds)
        local bossNid = LootContext.MarkActiveSnapshot(raidState, snapshot, now, ttlSeconds, LOOT_WINDOW_BOSS_CONTEXT_TTL_SECONDS)
        if bossNid <= 0 then
            return 0
        end
