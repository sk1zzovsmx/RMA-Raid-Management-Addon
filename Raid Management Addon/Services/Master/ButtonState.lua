...
function ButtonState.BuildTooltipState(opts)
	...
	local tooltipState = {
		...
		spamLoot = lootState.fromInventory
				and (opts.hasReadyCheckAccess and L.TipMasterReadyCheck or L.WarnReadyCheckNotAllowed)
			or L.TipMasterSpamLoot,
		...
		countdown = (opts.countdownRunning or lootState.rollStarted) and L.TipMasterCountdown
			or L.TipMasterCountdownInactive,
		...
		disenchant = lootState.disenchanter and L.TipMasterDisenchant:format(lootState.disenchanter)
			or L.TipMasterDisenchantUnset,
	...
	if not hasLootAccess then
		local itemActionWarning = lootState.fromInventory and (L.WarnInventoryTradeNoPermission or L.WarnMLOnlyMode)
			or L.WarnMLOnlyMode
		local hasItemActionAccess = lootState.fromInventory and hasInventoryTradeAccess
		local hasSelectionAccess = (lootState.fromInventory and hasInventoryTradeAccess)
			or ((not lootState.fromInventory) and hasLootSelectionAccess)
...
function ButtonState.BuildState(opts)
	...
	return {
		...
		canCountdown = hasItemActionAccess
			and (tonumber(lootState.lootCount) or 0) >= 1
			and opts.hasItem
			and (lootState.rollStarted or countdownRunning),
