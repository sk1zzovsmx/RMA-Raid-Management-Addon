...
local InternalEvents = Events and Events.Internal or {}
...
local function ensureRaidStore(raidNum)
    ...
    if Database.EnsureRaidById then
...
local function ensureState()
    local featureManualTrade = feature.lootState and feature.lootState.manualTrade or {}
    feature.lootState = feature.lootState or {}
    feature.lootState.manualTrade = featureManualTrade
...
local function findLocalHoldRows(raidNum)
    ...
    local currentPlayer = type(Database.GetPlayerName) == "function" and normalizeName(Database.GetPlayerName()) or nil
...
local function logCandidate(state, candidate, reason, raidId)
    ...
    if request.ok == true then
        ...
        if addon.hasDebug then
            addon:debug(Diag.D.LogManualTradeLogged:format(tostring(lootNid), tostring(state.partnerName), tostring(reason)))
        end
    ...
    if addon.hasWarn then
        addon:warn(Diag.E.LogTradeLoggerLogFailed:format(tostring(raidId), tostring(lootNid), tostring(candidate and candidate.itemLink)))
    end
    return false
end
...
function Trade.RefreshCandidate(opts)
    ...
    if matchedAny then
        ...
        if addon.hasDebug then
            addon:debug(Diag.D.LogManualTradeCandidateFound:format(tostring(partnerName), tostring(#state.candidates), tostring(state.candidateLootNid)))
        end
        return state
    end
