...
local sendAddonWhisper = Comms and Comms.SendAddonWhisper or function()
    return false
end
...
local function getIncomingNow()
    local timeFn = _G.GetTime
    local now = type(timeFn) == "function" and timeFn() or nil
...
local function sendData(target, requestId, format)
    ...
    if totalChunks > MAX_OUTGOING_CHUNKS then
        sendAddonWhisper(PREFIX, target, payloadCodec.PackFields(FIELD_SEP, MSG_DATA_ERR, tostring(requestId), "payload_too_large"))
        return false
    end
