...
local function findReasonLabel(masterService, reason)
	local reasonOrder = masterService
			and masterService.Trade
			and masterService.Trade.GetReasonOrder
			and masterService.Trade.GetReasonOrder()
		or nil
...
local function positionDropdown(dropdown, slot, parent)
	...
	if slotFrame then
		dropdown:SetPoint(
			...
			slotFrame,
			"BOTTOMRIGHT",
			-REASON_DROPDOWN_BUTTON_RIGHT_OVERHANG,
			-REASON_DROPDOWN_BUTTON_BOTTOM_OVERHANG
		)
	else
...
local function initDropdown(dropdown, candidate, masterService)
	...
	if UIDropDownMenu_Initialize then
		UIDropDownMenu_Initialize(dropdown, function(frameOrLevel, level)
			...
			local reasonOrder = masterService.Trade.GetReasonOrder()
			if menuLevel ~= 1 then
				return
			end
			for i = 1, #reasonOrder do
				...
				info.func = function(_, argLootNid, argReason)
					...
					masterService.Trade.SetSelectedReason(argLootNid, argReason)
					if candidate then
...
local function buildDropdown(slot, reasonOrder)
	...
	local parent = _G.TradeFrame or UIParent
	if not dropdown then
...
function module.RefreshDropdowns(state)
	...
	for slot = 1, 6 do
		local dropdown = module._dropdowns[slot] or buildDropdown(slot, MasterService.Trade.GetReasonOrder())
		if not dropdown then
