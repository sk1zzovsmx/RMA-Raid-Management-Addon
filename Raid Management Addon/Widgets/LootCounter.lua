...
-- Lua owns dynamic LootCounter section, header, and row construction because
...
do
	...
	local getFrame = Frames.MakeModuleFrameGetter(module, "RMALootCounterFrame")
	...
	function uiState.AcquireRefs(frame)
		local refs = {
			scrollFrame = frame
					and (frame.ScrollFrame or _G[(frame.GetName and frame:GetName() or "RMALootCounterFrame") .. "ScrollFrame"])
				or nil,
		...
		refs.scrollChild = (refs.scrollFrame and refs.scrollFrame.ScrollChild)
			or _G["RMALootCounterFrameScrollFrameScrollChild"]
		return refs
	end
	...
	local function ensureResetAllPopup()
		if IsPopupDefined(RESET_ALL_POPUP_KEY) then
			return true
		end

		return DefineConfirmPopup(RESET_ALL_POPUP_KEY, L.StrConfirmLootCounterResetAll, resetAllCounts, nil, {
			preferredIndex = 3,
		})
	end
	...
	local function canBroadcastCounter()
		...
		local state = GetCapabilityState(Raid, "loot_counter_broadcast")
	...
	local function ensureFrames()
		...
		scrollFrame = scrollFrame
			...
			or _G[uiState.FrameName .. "ScrollFrame"]
			or _G["RMALootCounterFrameScrollFrame"]
	...
	local function getCurrentRaidPlayers()
		...
		return GetLootCounterRows(Raid, Database.GetCurrentRaid(), raidPlayers)
	end
	...
	local function ensureRow(i, rowHeight)
		...
		if not row then
			...
			local function makeBtn(parent, label, tip)
				...
				Tooltips.Bind(b, tip, "ANCHOR_RIGHT")
				return b
			end
			...
			for lootType, sec in pairs(LOOT_TYPES) do
				local lt = lootType
				Frames.SetScriptSafely(sec.plus, "OnClick", function()
					local nid = row._playerNid
					if nid then
						AddPlayerLootCountByNid(Raid, nid, lt, 1, Database.GetCurrentRaid())
						module:RequestRefresh("count_changed")
					end
				end)
				Frames.SetScriptSafely(sec.minus, "OnClick", function()
					local nid = row._playerNid
					if nid then
						AddPlayerLootCountByNid(Raid, nid, lt, -1, Database.GetCurrentRaid())
						module:RequestRefresh("count_changed")
			...
			Frames.SetScriptSafely(row.reset, "OnClick", function()
				local nid = row._playerNid
				if nid then
					local raidNum = Database.GetCurrentRaid()
					SetPlayerLootCountByNid(Raid, nid, "ms", 0, raidNum)
					SetPlayerLootCountByNid(Raid, nid, "os", 0, raidNum)
					SetPlayerLootCountByNid(Raid, nid, "free", 0, raidNum)
	...
	function module:AttachToMaster(masterFrame)
		return attachCounterToMaster(masterFrame)
	end
		...
		for i = 1, #players do
			...
			if playerNid then
				...
				if ms ~= 0 or os ~= 0 or free ~= 0 then
					SetPlayerLootCountByNid(Raid, playerNid, "ms", 0, currentRaid)
					SetPlayerLootCountByNid(Raid, playerNid, "os", 0, currentRaid)
					SetPlayerLootCountByNid(Raid, playerNid, "free", 0, currentRaid)
					changed = true
				end
			end
		end
	...
	local function confirmResetAllCounts()
		if ensureResetAllPopup() then
			ShowPopup(RESET_ALL_POPUP_KEY)
		end
	end
	...
	local function refreshLootCounter()
		...
		for i = 1, numPlayers do
			...
			local class = data and data.class or GetPlayerClass(Raid, name)
			...
			local specIcon = nil
			if name then
				local spec = GetPlayerSpecSnapshot(SpecInspect, name)
	...
	RegisterCallback(RaidRosterDeltaEvent, requestRefresh)

	-- Refresh when counts actually change (MS loot award or manual +/-/reset).
	RegisterCallback(PlayerCountChangedEvent, requestRefresh)

	-- Refresh when spec inspection updates.
	RegisterCallback(SpecInspectUpdatedEvent, requestRefresh)
